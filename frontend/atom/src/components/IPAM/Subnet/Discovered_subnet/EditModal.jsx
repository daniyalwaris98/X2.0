...
const EditModal = (props) => {
  ...
  const postDevice = async (device) => {
    try {
      //console.log(device);
      await axios
        .post(baseUrl + "/editSubnetInDiscoveredSubnets ", device)
        .then((response) => {
          if (response?.response?.status == 500) {
            openSweetAlert(response?.response?.data?.response, "error");
          } else {
            openSweetAlert("Subnet Name Updated Successfully", "success");
            const promises = [];
            promises.push(
              axios
  ...
  //   );
  let [subnet_name, setSubnet_name] = useState(
