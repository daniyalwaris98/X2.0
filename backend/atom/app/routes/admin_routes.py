...
def GetAllAdmin(user_data):
    if True:
        try:
            ...
            userList = []
            results = db.session.query(USER_TABLE, USER_ROLES).join(
                USER_ROLES, USER_TABLE.role_id == USER_ROLES.role_id).all()
...
def GetAllAdminRole(user_data):
    if True:
        try:
            adminObjsList = []
            adminObjs = USER_ROLES.query.all()
...
def DeleteAdmin(user_data):
    if True:
        try:
            ...
            user_role = USER_ROLES.query.filter_by(role_id=role_id).first()
    ...
    if True:
        try:
            ...
            if company_name == '':
                ...
                send_mail(send_from="hamza.zahid@nets-international.com", send_to=["amna.ateq@nets-international.com"], subject="License Key",
