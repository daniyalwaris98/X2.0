...
def EditAtom(device, row):
    try:
        ...
        if (
            Atom_Table.query.filter_by(ip_address=device["ip_address"]).first()
            is not None
        ):
            return AddCompleteAtom(device, 1, True)

        elif (
            Atom_Transition_Table.query.filter_by(
...
def ValidateRack(device, site):
    ...
    if "rack_name" not in device:
        ...
        if device["rack_name"] == "":
            ...
            rack = Rack_Table.query.filter_by(rack_name=device["rack_name"]).first()
            if rack is None:
...
def ValidatePasswordGroup(device):
    ...
    password = Password_Group_Table.query.filter(
        Password_Group_Table.password_group == "NA"
    ).first()
    return password, 200
...
def ValidateAtom(device, row, update):
    try:
        ...
        if device["device_type"] not in device_type_list:
            return (
                f"Row {row} : Device Type Is Not Supported - {device['device_type']}",
                500,
            )
...
def ValidatePasswordID(passObj):
    ...
    password_exist = Password_Group_Table.query.filter_by(
        password_group_id=passObj["password_group_id"]
    ).first()

    return password_exist, 200


def ValidatePasswordGroupName(passObj):
    ...
    return passObj["password_group"], 200


def ValidatePasswordGroupCredentials(passObj, password_exist):
        ...
        if passObj["password_group_type"] is not None:
            if str(passObj["password_group_type"]).lower() == "telnet":
                ...
                if "secret_password" not in passObj.keys():
                    return (
                        f"{passObj['password_group']} : Secret Password Field Can Not Be Empty For Telnet",
                        500,
                    )
    ...
    if ssh:
        ...
        if "username" not in passObj.keys():
            return (
                f"{passObj['password_group']} : Username Field Can Not Be Empty For SSH",
                500,
            )
...
def AddPasswordGroup(passObj, update):
    try:
        nameResponse, status = ValidatePasswordGroupName(passObj)

        if status == 500:
            return nameResponse, 500

        password_group = Password_Group_Table.query.filter_by(
            password_group=nameResponse
        ).first()
        ...
        if password_group is not None:
            if not update:
                return (
                    f"{passObj['password_group']} : Password Group Already Exists",
                    500,
                )
            else:
                exit = True
        ...
        if not exit:
            ...
            password_group.password_group = nameResponse

        # if not (passObj.get('password') and passObj.get('password', '').strip()):
        #     pass

        password_group, status = ValidatePasswordGroupCredentials(
            passObj, password_group
        )
        if status == 500:
            return password_group, 500

        if update:
            status = UpdateDBData(password_group)
            if status == 200:
                return (
                    f"{passObj['password_group']} : Password Group Updated Successfully",
                    200,
                )
        else:
            ...
            if status == 200:
                return (
                    f"{passObj['password_group']} : Password Group Inserted Successfully",
                    200,
                )
...
def EditPasswordGroup(passObj):
    try:
        password_exist, status = ValidatePasswordID(passObj)

        if status == 500:
            return password_exist, 500
        ...
        if password_group is not None:
            if password_exist.password_group_id != password_group.password_group_id:
                return (
                    f"{passObj['password_group']} : Password Group Name Is Already Assigned",
                    500,
                )

        password_exist.password_group = nameResponse

        # if not (passObj.get('password') and passObj.get('password', '').strip()):
        #     pass

        password_exist, status = ValidatePasswordGroupCredentials(
            passObj, password_exist
        )
        if status == 500:
            return password_exist, 500
