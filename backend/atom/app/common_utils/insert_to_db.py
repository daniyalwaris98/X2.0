...
from app.models.inventory_models import SNTC_TABLE, Atom, Board_Table, Device_Table, License_Table, Phy_Table, Rack_Table, Sfps_Table, Subboard_Table,APS_TABLE
import traceback
na = "N/A"
tbf = "TBF"
...
def InsertUamDeviceData(data, atom, ip_addr):
                # print("$$$$$$$$ INSERT UAM DEVICE DATA ", file=sys.stderr)
                deviceObj = Device_Table()
                deviceObj.device_name = atom.device_name
                deviceObj.site_name = atom.site_name
                deviceObj.rack_name = atom.rack_name
                deviceObj.ip_address = ip_addr
                ...
                if "device" in data:
                    ...
                    if data['device']['status'] is not None:
                        ...
                        deviceObj.status= na
                    if atom.device_ru is not None:
                        deviceObj.ru= atom.device_ru
                    if atom.department is not None:
                        deviceObj.department= atom.department
                    else:
                        deviceObj.department = tbf
                    if atom.section is not None:
                        deviceObj.section= atom.section
                    else:
                        deviceObj.section = tbf
                    # if atom.criticality is not None:
                    #     deviceObj.criticality= atom.criticality
                    # else:
                    #     deviceObj.criticality = tbf
                    if atom.function is not None:
                        deviceObj.function= atom.function
                    else:
                        deviceObj.function = tbf
                    # if atom.domain is not None:
                    #     deviceObj.domain= atom.domain
                    # else:
                    #     deviceObj.domain = tbf
                    ...
                    deviceName = Device_Table.query.with_entities(Device_Table.device_name).filter_by(ip_address=deviceObj.ip_address).first()
                    if deviceName is not None:
                        deviceObj.device_name = deviceName[0]
                        print("Updated device " + deviceObj.ip_address,file=sys.stderr)
                        deviceObj.modification_date= datetime.now()
                        run = UpdateData(deviceObj)

                    else:
                        print("Inserted device " +deviceObj.ip_address,file=sys.stderr)
                        deviceObj.creation_date= datetime.now()
                        deviceObj.modification_date= datetime.now()
                        run = InsertData(deviceObj)
                    if run:
                        deviceName = Device_Table.query.with_entities(Device_Table.device_name).filter_by(ip_address=deviceObj.ip_address).first()[0]
                    return deviceName
                else:
...
def InsertUamDeviceBoardData(deviceName, data):
                ...
                for board in data['board']:
                    ...
                    if boardObj.serial_number:
                        boardName = Board_Table.query.with_entities(Board_Table.board_name).filter_by(serial_number=boardObj.serial_number).first()
                    else:
                        boardName = Board_Table.query.with_entities(Board_Table.board_name).filter_by(board_name=boardObj.board_name).first()
...
def InsertUamDeviceSubBoardData(deviceName, data):
                ...
                for subboard in data['sub_board']:
                    ...
                    if subboardObj.serial_number:
                        subboardName = Subboard_Table.query.with_entities(Subboard_Table.subboard_name).filter_by(serial_number=subboardObj.serial_number).first()
                    else:
                        subboardName = Subboard_Table.query.with_entities(Subboard_Table.subboard_name).filter_by(subboard_name=subboardObj.subboard_name).first()
...
def InsertUamDeviceSfpData(deviceName,data):
                ...
                for sfp in data['sfp']:
                    ...
                    sfpObj = Sfps_Table.query.with_entities(Sfps_Table).filter_by(serial_number=sfpData.serial_number).first()
...
def InsertUamDeviceLicenseData(device_name, data):
                ...
                for license in data['license']:
                    if license['name'] != "":
                        ...
                        licenseName = License_Table.query.with_entities(License_Table.license_name).filter_by(license_name=licenseData.license_name).first()
    ...
    try:
        ...
        for ip_addr in pullerData:
            ...
            if 'error' in data:
                ...
                site = Phy_Table.query.filter_by(site_name=atom.site_name).first()
        ...
        traceback.print_exc()
        print(f"Error while getting data from device {ip_addr} error {e}", file=sys.stderr)    
        error = True
        failed = True
    return failed