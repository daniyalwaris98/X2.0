...
from app.models.inventory_models import MONITORING_DEVICES_TABLE, MONITORING_FIREWALLS_DEVICES_TABLE, MONITORING_CREDENTIALS_TABLE, MONITORING_FIREWALLS_INTERFACES_TABLE, MONITORING_LINUX_DEVICES_TABLE, MONITORING_LINUX_INTERFACES_TABLE, MONITORING_NETWORK_DEVICES_TABLE, MONITORING_NETWORK_INTERFACES_TABLE, MONITORING_ROUTERS_DEVICES_TABLE, MONITORING_ROUTERS_INTERFACES_TABLE, MONITORING_SERVERS_DEVICES_TABLE, MONITORING_SERVERS_INTERFACES_TABLE, MONITORING_SWITCHES_DEVICES_TABLE, MONITORING_SWITCHES_INTERFACES_TABLE, MONITORING_WINDOWS_DEVICES_TABLE, MONITORING_WINDOWS_INTERFACES_TABLE, MONITORING_WIRELESS_DEVICES_TABLE, MONITORING_WIRELESS_INTERFACES_TABLE
...
def GetMonitoringDevicesCards(user_data):
    if True:
        try:
                    ...
                    for record in table.records:
                        ...
                        try:
                            if record['Response']:
                                ...
                                objDict3['value'] = round(
                                    float(record['Response']), 2)
                        ...
                        try:
                            if record['Memory']:
                                ...
                                objDict7['value'] = round(
                                    float(record['Memory']), 2)
            ...
            finalcardslist = [i for n, i in enumerate(cardslist) if i not in cardslist[n + 1:]]
...
def int_band():
    ...
    result = query_api.query(org='monetx', query=query)
    print("##############################%$^%^45printing result value ",result,file=sys.stderr)
    resulta = []
    objDict = {}
    upload = []
    download = []
    ...
    try:
        for table in result:
            for record in table.records:
                if record['Interface_Name'] == jsonObj['interface_name']:
                    ...
                    try:
                        objDict3['date'] = datetime.strptime(record['Date'], '%Y-%m-%d %H:%M:%S.%f').strftime('%Y-%m-%d %H:%M:%S')
        ...
        print(f"@@@@@@@@@#########download{download},Upload{upload},all{all}",file=sys.stderr)
        if len(download)>0  and len(upload)>0:
            ...
            final.append({"bandwidth": "Upload", "min": min(upload), "max": max(upload), "avg": round(sum(upload)/len(upload), 2)})
            final.append({"bandwidth": "Average", "min": min(all),"max": max(all), "avg": round(sum(all)/len(all), 2)})
            objDict['table'] = final
...
def GetAllMonitoringVendors(user_data):
    if True:
        try:
            queryString = f"select vendor,count(vendor) from monitoring_devices_table group by vendor;"
            result = db.session.execute(queryString)
            objList = []
            for row in result:
                objDict = {}
...
def GetAllMonitoringFunctions(user_data):
    if True:
        try:
            queryString = f"select `function`,count(`function`) from monitoring_devices_table group by `function`;"
            result = db.session.execute(queryString)
            objList = []
            for row in result:
                objDict = {}
