...
class FortinetPuller(object):
    fortinet_interfaces = None
    def __init__(self):
        self.fortinet_interfaces = dict()
        ...
        if con_exception ==None:
            ...
            if output['status'] == "Up":
                ...
                try:
                    ...
                    print(f"memory of fortinet: {output['memory']}",file=sys.stderr)
                ...
                # [i for n, i in enumerate(cardslist) if i not in cardslist[n + 1:]]
                    ...
                    for key in interfaces_val:   
                        try:     
                            self.fortinet_interfaces[interfaces_val[key][0]]={'Download': str(float(download_val[key][0])/1000000) ,'Upload': str(float(upload_val[key][0])/1000000),'Interface Status':interface_status[key][0],'Interface Description':interface_description[key][0]}
                        except Exception as e:
                            print("exception in ios interfaces dictionary:",str(e),file=sys.stderr)
                            pass
                    
                    print(f"222222222222222222222 data of final_interfaces,len:{len(self.fortinet_interfaces,)}",self.fortinet_interfaces, file=sys.stderr)
                    
                    if len(final_interfaces) == 0:
                        pass
                    else:
                        for interface in final_interfaces:
                            self.fortinet_interfaces[interface['interface_name']]['Download'] = round(float(((float(self.fortinet_interfaces[interface['interface_name']]['Download'])-float(interface['Download']))*8*1000) /bandwidth_time),2)
                            self.fortinet_interfaces[interface['interface_name']]['Upload'] = round(float(((float(self.fortinet_interfaces[interface['interface_name']]['Upload'])-float(interface['Upload']))*8*1000) /bandwidth_time),2)
                        
                    self.data_dumping(host)

                    print(f"///interfaces of {host[1]},count: {len(upload_val)}::",self.fortinet_interfaces,file=sys.stderr)
                    ...
                    self.data_dumping(host)
                    print(output,self.fortinet_interfaces, file=sys.stderr)                
        else :
    ...
    def data_dumping(self,host):
        ...
        try:
            ...
            return e
        if len(self.fortinet_interfaces.items()) > 0:
            print("in full interfaces block")


            for k in self.fortinet_interfaces.keys():
                dictionary1 = [{
                ...
                {
                "Interface_Name" : k,
                "Status" : self.fortinet_interfaces[k]['Interface Status'],
                "Download" : float(self.fortinet_interfaces[k]['Download']),
                "Upload" : float(self.fortinet_interfaces[k]['Upload']),
                "Interface_Des" :self.fortinet_interfaces[k]['Interface Description'],
                "Date": str(datetime.now())
                }}]
        
                # data = influxdb_client.Point.from_dict(dictionary1, WritePrecision.NS)

                # data = influxdb_client.Point("Interfaces").tag('IP_ADDRESS',host[1]).field('DEVICE_NAME',host[1]).field('Type',"Ciso Asa").field('Status',self.fortinet_interfaces[k]['Status'][0]).field('Download',float(self.fortinet_interfaces[k]['Download'])).field('Upload',float(self.fortinet_interfaces[k]['Upload']))
                try:
        ...
        return (self.fortinet_interfaces,output,datetime.now())
...
# print(output,self.fortinet_interfaces)
