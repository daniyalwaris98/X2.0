...
oids = {
        ...
        'CPU Utilization':'1.3.6.1.4.1.14179.1.1.5.1',
        'interfaces':'1.3.6.1.2.1.2.2.1.2',
        'Memory Utilization': '1.3.6.1.4.1.9.9.618.1.8.6',
        'temprature':'1.3.6.1.4.1.14179.2.3.1.13',
        'Access Points' : '1.3.6.1.4.1.9.9.618.1.8.4',
        'Clients': '1.3.6.1.4.1.9.9.618.1.8.12',
...
class WLCSPuller(object):
    wlc_interfaces = None
    def __init__(self):
        self.wlc_interfaces = dict()
    def poll(self,host):
        ...
        try:
            ...
            if host[14] == "v3":
                ...
                print(f"3333333333333 print values in v3 wlc poll userName= {host[21]}, authKey={host[22]},privKey={host[23]}, authProtocol={auth_proc}, privProtocol={encryp_proc}",file=sys.stderr)
        ...
        if con_exception ==None:
            ...
            if output['status'] == 'Up':
                ...
                try:
                    ...
                    print(f"i am in uptime exception {e}",host,file=sys.stderr)
                    output['Uptime'] = 'NA'

                #for CPU Utilization utilization
                try:
                ...
                print(f"cpu of wlc: {output['cpu']}",file=sys.stderr)

                #for memory utilization
                
                try:
                    temp = get_oid_data(engn, community, transport, cnxt, oids['Memory Utilization'])
                    print("i am print output of temp",temp, file= sys.stderr)
                ...
                print(f"memory of wlc: {output['memory']}",file=sys.stderr)


                #for Temprature
                try:
                    temp = get_oid_data(engn, community, transport, cnxt, oids['temprature'])
                    output['Temprature'] = str(general(temp))
                except Exception as e:
                    print(e)
                    output['Temprature'] = 'NA'
                
                #for Aceess Points
                try:
                    temp = get_oid_data(engn, community, transport, cnxt, oids['Access Points'])
                    output['Aceess Points'] = str(general(temp))
                except Exception as e:
                    print(e)
                    output['Aceess Points'] = 'NA'
                #for Clients
                try:
                    temp = get_oid_data(engn, community, transport, cnxt, oids['Clients'])
                    output['Clients'] = str(general(temp))
                except Exception as e:
                    print(e)
                    output['Clients'] = 'NA'
                    # device description
                ...
                # [i for n, i in enumerate(cardslist) if i not in cardslist[n + 1:]]
                    ...
                    for key in interfaces_val:   
                        try:     
                            self.wlc_interfaces[interfaces_val[key][0]]={'Download': str(float(download_val[key][0]/1000000)) ,'Upload': str(float(upload_val[key][0]/1000000)),'Interface Status':interface_status[key][0],'Interface Description':interface_description[key][0]}
                        except Exception as e:
                            print("exception in ios interfaces dictionary:",str(e),file=sys.stderr)
                            pass
                    
                    print(f"222222222222222222222 data of final_interfaces,len:{len(self.wlc_interfaces,)}",self.wlc_interfaces, file=sys.stderr)
                    
                    if len(final_interfaces) == 0:
                        pass
                    else:
                        for interface in final_interfaces:
                            self.wlc_interfaces[interface['interface_name']]['Download'] = round(float(((float(self.wlc_interfaces[interface['interface_name']]['Download'])-float(interface['Download']))*8*1000) /bandwidth_time),2)
                            self.wlc_interfaces[interface['interface_name']]['Upload'] = round(float(((float(self.wlc_interfaces[interface['interface_name']]['Upload'])-float(interface['Upload']))*8*1000) /bandwidth_time),2)
                        
                    self.data_dumping(host)

                    print(f"///interfaces of {host[1]},count: {len(upload_val)}::",self.wlc_interfaces,file=sys.stderr)
            else:
                    output['Uptime'] = 'NA'
                    output['cpu'] = 'NA'
                    output['memory'] = 'NA'
                    ...
                    self.data_dumping(host)
                    print(output,self.wlc_interfaces, file=sys.stderr)                 
        else:
    ...
    def data_dumping(self,host):
        ...
        dictionary = [
            ...
            "Access_Points" : output['Aceess Points'],
            "Clients" : output['Clients'],
            "Temprature" : output['Temprature'],
        ...
        try:
            ...
            return e
        if len(self.wlc_interfaces.items()) > 0:
            print("in full interfaces block")


            for k in self.wlc_interfaces.keys():
                dictionary1 = [{
                ...
                {
                "Interface_Name" : k,
                "Status" : self.wlc_interfaces[k]['Interface Status'],
                "Download" : float(self.wlc_interfaces[k]['Download']),
                "Upload" : float(self.wlc_interfaces[k]['Upload']),
                "Interface_Des" :self.wlc_interfaces[k]['Interface Description'][0],
                "Date": str(datetime.now())
